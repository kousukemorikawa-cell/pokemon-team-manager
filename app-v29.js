...
function megaStoneFor(name){const n=normalizePokemonName(name);return MEGA_STONES[n]||(MEGA_NAMES.has(n)?'メガストーン':'')}
  ...
  function normalizePokemonName(name){return String(name||'').trim().replace(/\s+/g,'').replace(/[・･]/g,'').replace(/（/g,'(').replace(/）/g,')').replace(/[Ｘｘ]/g,'X').replace(/[Ｙｙ]/g,'Y').replace(/イダイトウ(?:オス)?$/,'イダイトウ♂').replace(/イダイトウメス$/,'イダイトウ♀')}
  ...
  async function openPokemonPage(name,returnView=null){
    name=normalizePokemonName(name);state.currentPokemon=name;state.pokemonReturnView=returnView;hideAllViews();$('#pokemonView').classList.add('active');$('#pokemonPageTitle').textContent=state.historyPick?`${name}：過去の型を選択`:`${name}のまとめ`;await renderPokemonPage();scrollTo(0,0)
  }
  ...
  function selectedTeamButton(kind,select){const btn=document.createElement('button');btn.type='button';btn.className='team-select-visual';btn.dataset.openTeamPicker=kind;const team=state.teams.find(t=>t.id===select.value);btn.innerHTML=team?`<span><b>${esc(teamLabel(team))}</b><span class="team-select-images">${team.pokemon.filter(Boolean).map(n=>`<img data-pokemon="${esc(n)}" src="${imgFor(n)}">`).join('')}</span></span><em>変更</em>`:`<span>画像一覧から${kind==='self'?'自分':'相手'}の構築を選択</span><em>選択</em>`;select.hidden=true;select.after(btn)}
  ...
  function renderTeamPicker(q=''){const query=searchNorm(q),rows=state.teams.filter(t=>(state.teamPickerKind!=='self'||t.owner==='self')&&(!query||searchNorm([t.title,t.user,t.rank,...t.pokemon].join(' ')).includes(query)));$('#teamPickerList').innerHTML=rows.length?rows.map(t=>`<article class="team-picker-card"><div><b>${esc(teamLabel(t))}</b><div class="team-picker-images">${t.pokemon.filter(Boolean).map(n=>`<img data-pokemon="${esc(n)}" src="${imgFor(n)}">`).join('')}</div></div><button class="team-picker-check" data-pick-team="${t.id}" aria-label="この構築を選択">✓</button></article>`).join(''):'<div class="empty">該当する構築がありません。</div>';hydrateImages($('#teamPickerList'))}
  function openTeamPicker(kind){state.battleFormCache=captureBattleForm();state.teamPickerKind=kind;hideAllViews();$('#teamPickerView').classList.add('active');$('#teamPickerTitle').textContent=kind==='self'?'自分の構築を選択':'相手の構築を選択';$('#teamPickerSearch').value='';renderTeamPicker();scrollTo(0,0)}
  ...
  document.addEventListener('click',e=>{const move=e.target.closest?.('[data-save-move]');if(move){e.preventDefault();e.stopImmediatePropagation();saveMoveWithoutReset(move);return}if(e.target.closest?.('#runDamageCalc')){e.preventDefault();e.stopImmediatePropagation();calculateAndStoreDamage();return}if(e.target.closest?.('#saveBattleMemo')){e.preventDefault();e.stopImmediatePropagation();saveBattleRecordV27();return}const picker=e.target.closest?.('[data-open-team-picker]');if(picker){openTeamPicker(picker.dataset.openTeamPicker);return}const picked=e.target.closest?.('[data-pick-team]');if(picked){if(state.teamPickerKind==='self')state.battleFormCache.selfId=picked.dataset.pickTeam;else state.battleFormCache.opponentId=picked.dataset.pickTeam;openBattleTool(state.battleFormCache.selfId);return}if(e.target.closest?.('#teamPickerBackBtn,#damageHistoryBackBtn,#battleHistoryBackBtn')){openBattleTool(state.battleFormCache?.selfId||'');return}if(e.target.closest?.('#openDamageHistory')){openDamageHistory();return}if(e.target.closest?.('#openBattleHistory')){openBattleHistory();return}},true);
  ...
  function damageCalculatorHTML(){return `<div class="section damage-calculator-section"><div class="section-title-row"><h3>ダメージ計算</h3><div class="toolbar"><button type="button" class="btn" id="openDamageHistory">計算履歴</button><button id="runDamageCalc" class="btn primary">計算する</button></div></div><p class="sub">Lv.50・個体値31。努力値は0〜32。攻撃・防御補正は能力ランク（−6〜＋6）から選択できます。</p><div class="damage-columns"><div class="damage-side"><h3>攻撃側</h3>${damageMonPanel('atk',0)}${damageMonPanel('atk',1)}<button type="button" class="btn small add-damage-mon" data-toggle-damage-panel="atk">＋ 攻撃側2匹目を追加</button></div><div class="damage-side"><h3>防御側</h3>${damageMonPanel('def',0)}${damageMonPanel('def',1)}<button type="button" class="btn small add-damage-mon" data-toggle-damage-panel="def">＋ 防御側2匹目を追加</button></div></div><div id="damageResult" class="damage-result">入力後「計算する」を押してください。</div></div>`}
  ...
  function openDamageTool(){state.damageHistoryReturn='damage';hideAllViews();$('#battleToolView').classList.add('active');$('#battleToolContent').innerHTML=damageCalculatorHTML();prepareDamageCalculator($('#battleToolContent'));restoreBattleForm(state.battleFormCache);syncSecondaryDamagePanels($('#battleToolContent'));scrollTo(0,0)}
  ...
  function renderBattlePartyPickers(){const host=$('#battlePartyPokemon');if(!host)return;const group=(title,teamId,side)=>{const t=state.teams.find(x=>x.id===teamId),names=(t?.pokemon||[]).filter(Boolean);return `<div class="battle-party-picker"><h4>${title}</h4><div>${names.length?names.map(n=>`<button type="button" data-party-damage-pokemon="${esc(n)}" data-party-damage-side="${side}"><img data-pokemon="${esc(n)}" src="${imgFor(n)}"><span>${esc(n)}</span></button>`).join(''):'<span class="sub">構築を選択してください</span>'}</div></div>`};host.innerHTML=group('自分の構築',$('#battleSelfTeam')?.value,'atk')+group('相手の構築',$('#battleOpponentTeam')?.value,'def');host.querySelectorAll('[data-party-damage-pokemon]').forEach(btn=>btn.onclick=()=>setDamagePokemon(btn.dataset.partyDamageSide,btn.dataset.partyDamagePokemon));hydrateImages(host)}
  ...
  async function calculateBattleDamage(){
    ...
    for(let i=0;i<atkPanels.length;i++){
      ...
      sums[target].min+=range.min;sums[target].max+=range.max;updateDamagePanelPreview(panel);lines.push(`${data.pokemon}の${data.moveName||'技'} → ${def.data.pokemon}：${range.min}〜${range.max}（${(range.min/hp*100).toFixed(1)}〜${(range.max/hp*100).toFixed(1)}%）`)
    }
    for(let i=0;i<2;i++)if(sums[i].max)lines.push(`防御側${i+1}への合計：${sums[i].min}〜${sums[i].max}ダメージ`);
    $('#damageResult').innerHTML=lines.map(x=>`<div>${esc(x)}</div>`).join('')||'計算できる入力がありません。'
  }
  async function calculateAndStoreDamage(){await calculateBattleDamage();const result=$('#damageResult')?.innerText.trim()||'';if(!result||result.includes('計算できる'))return;const attackers=[...document.querySelectorAll('[data-damage-atk]')].map(p=>p.querySelector('[data-dmg="pokemon"]')?.value).filter(Boolean),defenders=[...document.querySelectorAll('[data-damage-def]')].map(p=>p.querySelector('[data-dmg="pokemon"]')?.value).filter(Boolean),moves=[...document.querySelectorAll('[data-damage-atk]')].map(p=>p.querySelector('[data-dmg="moveName"]')?.value).filter(Boolean);state.damageHistory.unshift({id:uid(),date:new Date().toLocaleString('ja-JP'),attackers,defenders,pokemon:[...attackers,...defenders],moves,result});scheduleSave();toast('計算履歴に保存しました')}
  ...
  function damageHistoryCard(h){const sides=historySides(h);return `<article class="history-result-card damage-history-card"><div class="damage-history-sides"><div><b>攻撃側</b><div>${historyPokemonImages(sides.attackers)}</div></div><span class="damage-history-arrow">→</span><div><b>防御側</b><div>${historyPokemonImages(sides.defenders)}</div></div></div><small>${esc(h.date)} ／ ${esc((h.moves||[]).filter(Boolean).join('・')||'技名未登録')}</small><div>${esc(h.result)}</div></article>`}
