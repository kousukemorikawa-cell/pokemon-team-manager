...
function megaStoneFor(name){const n=normalizePokemonName(name);return MEGA_STONES[n]||(MEGA_NAMES.has(n)?'メガストーン':'')}
  ...
  function renderFilters(){const defs=[['all','すべて'],['red','🔴 注目'],['blue','🔵 試用'],['green','🟢 上位'],['purple','🟣 要研究']],host=$('#filters'),specialActive=state.someoneTeamsOnly||state.recentTeamsOnly;host.innerHTML=`<button class="btn small scope-filter ${state.teamScope==='my'&&!specialActive?'active':''}" data-scope-filter="my">My Team</button><button class="btn small scope-filter ${state.teamScope==='world'&&!specialActive?'active':''}" data-scope-filter="world">World Team</button>`+defs.map(([k,l])=>`<button class="btn small filter ${state.filter===k&&!specialActive?'active':''}" data-filter="${k}">${l}</button>`).join('')+`<button class="btn small someone-teams-tab ${state.someoneTeamsOnly&&!state.recentTeamsOnly?'active':''}" data-someone-tab>誰かの構築</button><button class="btn small recent-teams-btn ${state.recentTeamsOnly?'active':''}" data-recent-tab>最近使った構築</button>`;host.querySelectorAll('[data-scope-filter]').forEach(b=>b.onclick=()=>{state.teamScope=b.dataset.scopeFilter;state.someoneTeamsOnly=false;state.recentTeamsOnly=false;scheduleSave();renderTeamScopeTabs();renderList();renderFilters()});host.querySelectorAll('[data-filter]').forEach(b=>b.onclick=()=>{state.filter=b.dataset.filter;state.someoneTeamsOnly=false;state.recentTeamsOnly=false;renderList();renderFilters()});host.querySelector('[data-someone-tab]').onclick=()=>{state.someoneTeamsOnly=!state.someoneTeamsOnly;state.recentTeamsOnly=false;renderList();renderFilters()};host.querySelector('[data-recent-tab]').onclick=()=>{state.recentTeamsOnly=!state.recentTeamsOnly;if(state.recentTeamsOnly)state.someoneTeamsOnly=false;renderList();renderFilters()}}
  function filtered(){
