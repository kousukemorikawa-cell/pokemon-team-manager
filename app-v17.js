(() => {
  ...
  const state={teams:[], pairings:[], imageMap:{}, seasons:['未分類'], currentSeason:'all', teamScope:'world', filter:'all', query:'', sortOrder:'manual', currentId:null, currentPokemon:'', detailReturn:null, historyPick:null,currentPairingId:null,pairingMode:'browse',pairingReturnTeamId:null,pairingQuery:''};
...
function megaStoneFor(name){const n=normalizePokemonName(name);return MEGA_STONES[n]||(MEGA_NAMES.has(n)?'メガストーン':'')}
  ...
  function payload(){return {teams:state.teams,pairings:state.pairings,seasons:state.seasons,currentSeason:state.currentSeason,teamScope:state.teamScope,sortOrder:state.sortOrder,updatedAt:new Date().toISOString()}}
  ...
  const blankTeam=()=>({id:uid(),user:'',owner:state.teamScope==='my'?'self':'other',rank:'',season:state.currentSeason==='all'?(state.seasons[0]||'未分類'):state.currentSeason,regulation:'',title:'',date:todayText(),tag:'none',updatedAt:new Date().toISOString(),pokemon:Array(6).fill(''),items:Array(6).fill(''),abilities:Array(6).fill(''),natures:Array(6).fill(''),pokemonChanges:[],moveChanges:[],adjustments:Array.from({length:6},blankAdjustment),favorableMatchups:[],unfavorableMatchups:[],relatedPairingIds:[],overallMemo:{lead:'先：\n後：',weak:'',improve:'',battle:'',free:''},partyView:'overview'});
  ...
  function filtered(){
    ...
    const rows=state.teams.filter(t=>((state.teamScope==='my'?(t.owner==='self'):(t.owner!=='self')))&&(state.currentSeason==='all'||(t.season||'未分類')===state.currentSeason)&&(state.filter==='all'||t.tag===state.filter)&&(!q||[t.date,t.user,t.rank,t.title,t.season,...t.pokemon].join(' ').toLowerCase().includes(q)));
  ...
  function partyOverviewMon(t,i){const name=t.pokemon[i]||'',item=t.items[i]||'',ability=t.abilities[i]||'',nature=t.natures[i]||'';return `<article class="party-overview-mon" data-party-card="${i}"><div class="overview-image-wrap"><img data-pokemon="${esc(name)}" src="${imgFor(name)}"><div class="type-badges" data-type-for="${esc(name)}"></div><img class="held-item-icon large" data-item="${esc(item)}" alt=""></div><strong class="overview-name">${esc(name||'未登録')}</strong><div class="overview-line">${esc(item||'持ち物未登録')}｜${esc(ability||'特性未登録')}</div><div class="overview-nature ${natureClass(nature)}">${esc(natureLabel(nature)||'性格未登録')}</div><div class="party-stat-summary compact" data-party-stat="${i}">調整後：H－, A－, B－, C－, D－, S－</div><div class="party-effort-summary compact" data-party-effort="${i}">振り方：H0, A0, B0, C0, D0, S0</div></article>`}
  ...
  async function openPokemonPage(name){
    state.currentPokemon=name;hideAllViews();$('#pokemonView').classList.add('active');$('#pokemonPageTitle').textContent=state.historyPick?`${name}：過去の型を選択`:`${name}の登録一覧`;await renderPokemonPage();scrollTo(0,0)
  }
  async function renderPokemonPage(){
    const name=state.currentPokemon,occ=state.historyPick?adjustmentHistoryEntries(name,state.historyPick.teamId,state.historyPick.index):pokemonOccurrences(name),content=$('#pokemonPageContent');
    ...
    hydrateImages(content);content.querySelectorAll('[data-open-team]').forEach(b=>b.onclick=()=>openDetail(b.dataset.openTeam,{type:'pokemon',name}));content.querySelectorAll('[data-pick-history]').forEach(b=>b.onclick=()=>applyHistoryPick(b.dataset.sourceTeam,+b.dataset.sourceIndex));
  ...
  function closePokemonPage(){if(state.historyPick){const pick=state.historyPick,ctx=pick.returnContext;state.historyPick=null;openDetail(pick.teamId,ctx);return}hideAllViews();$('#listView').classList.add('active');renderList();renderFilters();scrollTo(0,0)}

  function addTeam(){const t=blankTeam();state.teams.unshift(t);scheduleSave();openDetail(t.id)}

  const blankPairing=()=>({id:uid(),date:todayText(),name:'',pokemon:Array(6).fill(''),strength:'',counter:'',memo:'',updatedAt:new Date().toISOString()});
  function relatedPairingsSection(t){
    const rows=(t.relatedPairingIds||[]).map(id=>state.pairings.find(p=>p.id===id)).filter(Boolean);
    return `<div class="section"><div class="section-title-row"><h3>関連する並び</h3><button type="button" class="btn primary" id="selectRelatedPairingsBtn">一覧から複数選択</button></div><div class="related-pairings">${rows.length?rows.map(p=>`<button type="button" class="related-pairing-link" data-open-pairing="${p.id}"><span><strong>${esc(p.name||'名称未設定')}</strong><small>${esc(p.date||'')}</small></span><span class="related-pairing-images">${p.pokemon.filter(Boolean).slice(0,4).map(n=>`<img data-pokemon="${esc(n)}" src="${imgFor(n)}">`).join('')}</span></button>`).join(''):'<div class="sub">まだ関連する並びが選択されていません。</div>'}</div></div>`;
  }
  function showOnlyView(id){document.querySelectorAll('.view').forEach(v=>v.classList.toggle('active',v.id===id))}
  function openPairingList(mode='browse',teamId=null){state.pairingMode=mode;state.pairingReturnTeamId=teamId;state.pairingQuery='';showOnlyView('pairingView');const q=$('#pairingSearch');if(q)q.value='';renderPairingList();scrollTo(0,0)}
  function filteredPairings(){const q=searchNorm(state.pairingQuery||'');return [...state.pairings].filter(p=>!q||searchNorm([p.name,p.date,p.strength,p.counter,...p.pokemon].join(' ')).includes(q)).sort((a,b)=>dateSortValue(b.date)-dateSortValue(a.date)||String(b.updatedAt).localeCompare(String(a.updatedAt)))}
  function pairingCardHTML(p){const selecting=state.pairingMode==='select';const team=state.teams.find(t=>t.id===state.pairingReturnTeamId);const checked=!!team?.relatedPairingIds?.includes(p.id);return `<article class="pairing-entry"><div class="pairing-entry-head">${selecting?`<label class="pairing-check"><input type="checkbox" data-pairing-check="${p.id}" ${checked?'checked':''}></label>`:''}<strong>${esc(p.name||'名称未設定')}</strong><span class="pairing-date">${esc(p.date||'')}</span></div><div class="pairing-mons">${p.pokemon.filter(Boolean).map(n=>`<div class="pairing-mon"><img data-pokemon="${esc(n)}" src="${imgFor(n)}"><small>${esc(n)}</small></div>`).join('')||'<span class="sub">ポケモン未登録</span>'}</div><div class="pairing-notes"><div class="pairing-note"><b>強み</b>${esc(p.strength||'未入力')}</div><div class="pairing-note"><b>対策</b>${esc(p.counter||'未入力')}</div></div><div class="pairing-actions"><button class="btn small" data-view-pairing="${p.id}">閲覧・編集</button></div></article>`}
  function renderPairingList(){const list=filteredPairings();const selecting=state.pairingMode==='select';$('#pairingSelectionNotice').hidden=!selecting;$('#pairingSelectionDone').hidden=!selecting;$('#newPairingBtn').hidden=selecting;$('#pairingSelectionNotice').textContent=selecting?'この構築に関連する並びを複数選び、「選択を完了」を押してください。':'';$('#pairingList').innerHTML=list.length?list.map(pairingCardHTML).join(''):'<div class="empty">並びメモがありません。「＋追加」から登録してください。</div>';hydrateImages($('#pairingList'));document.querySelectorAll('[data-view-pairing]').forEach(b=>b.onclick=()=>openPairingEditor(b.dataset.viewPairing));document.querySelectorAll('[data-pairing-check]').forEach(c=>c.onchange=()=>{const t=state.teams.find(x=>x.id===state.pairingReturnTeamId);if(!t)return;t.relatedPairingIds=t.relatedPairingIds||[];if(c.checked&&!t.relatedPairingIds.includes(c.dataset.pairingCheck))t.relatedPairingIds.push(c.dataset.pairingCheck);if(!c.checked)t.relatedPairingIds=t.relatedPairingIds.filter(id=>id!==c.dataset.pairingCheck);scheduleSave()})}
  function openPairingEditor(id){state.currentPairingId=id;showOnlyView('pairingEditView');renderPairingEditor();scrollTo(0,0)}
  function renderPairingEditor(){const p=state.pairings.find(x=>x.id===state.currentPairingId);if(!p)return;$('#pairingEditTitle').textContent=p.name||'並びを編集';$('#pairingEditContent').innerHTML=`<div class="section"><h3>並びメモ</h3><div class="detail-grid"><div class="field"><label>日付</label><input type="date" data-pairing-field="date" value="${esc(String(p.date||'').replaceAll('/','-'))}"></div><div class="field"><label>並び名</label><input data-pairing-field="name" value="${esc(p.name)}" placeholder="例：カイリュー＋サーフゴー"></div></div></div><div class="section"><h3>ポケモン（最大6体）</h3><div class="pairing-editor-party">${p.pokemon.map((n,i)=>`<div class="pairing-editor-mon"><img data-pokemon="${esc(n)}" src="${imgFor(n)}"><input data-pairing-pokemon="${i}" value="${esc(n)}" placeholder="ポケモン名"></div>`).join('')}</div></div><div class="section"><div class="field"><label>強み</label><textarea class="auto-grow" data-pairing-field="strength">${esc(p.strength)}</textarea></div><div class="field"><label>対策</label><textarea class="auto-grow" data-pairing-field="counter">${esc(p.counter)}</textarea></div><div class="field"><label>自由メモ</label><textarea class="auto-grow" data-pairing-field="memo">${esc(p.memo)}</textarea></div></div>`;hydrateImages($('#pairingEditContent'));document.querySelectorAll('[data-pairing-field]').forEach(el=>{el.oninput=()=>{p[el.dataset.pairingField]=el.dataset.pairingField==='date'?String(el.value).replaceAll('-','/'):el.value;p.updatedAt=new Date().toISOString();scheduleSave();if(el.dataset.pairingField==='name')$('#pairingEditTitle').textContent=el.value||'並びを編集'};if(el.tagName==='TEXTAREA')enableAutoGrow(el)});document.querySelectorAll('[data-pairing-pokemon]').forEach(el=>{attachAutocomplete(el,'pokemon');el.onchange=()=>{p.pokemon[+el.dataset.pairingPokemon]=el.value.trim();p.updatedAt=new Date().toISOString();scheduleSave();renderPairingEditor()}})}
  function addPairing(){const p=blankPairing();state.pairings.unshift(p);scheduleSave();openPairingEditor(p.id)}
  function deleteCurrentPairing(){const p=state.pairings.find(x=>x.id===state.currentPairingId);if(!p||!confirm('この並びメモを削除しますか？'))return;state.pairings=state.pairings.filter(x=>x.id!==p.id);state.teams.forEach(t=>t.relatedPairingIds=(t.relatedPairingIds||[]).filter(id=>id!==p.id));scheduleSave();openPairingList()}
  ...
  async function loadCloudData(){
    try{const {doc,getDoc}=cloud.api;const snap=await getDoc(doc(cloud.db,'users',cloud.user.uid,'apps','pokemon-team-manager'));
      if(snap.exists()){
        ...
        if(useCloud){state.teams=d.teams||[];state.pairings=d.pairings||[];state.seasons=d.seasons||['未分類'];state.currentSeason=d.currentSeason||'all';state.teamScope=d.teamScope||'world';state.sortOrder=d.sortOrder||'manual';await dbSet('data',payload());normalizeState();renderSeasons();renderFilters();renderTeamScopeTabs();renderSortControl();renderList();if(state.currentId)renderDetail();toast('クラウドデータを読み込みました')}
