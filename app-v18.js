...
function megaStoneFor(name){const n=normalizePokemonName(name);return MEGA_STONES[n]||(MEGA_NAMES.has(n)?'メガストーン':'')}
  ...
  function filteredPairings(){const q=searchNorm(state.pairingQuery||'');return [...state.pairings].filter(p=>!q||searchNorm([p.name,p.date,p.strength,p.selection,p.counter,p.memo,p.metaStatus,p.season,...p.pokemon,...p.counterGroups.flat(),...p.complementGroups.flat(),...p.strongAgainstGroups.flat()].join(' ')).includes(q)).sort((a,b)=>dateSortValue(b.date)-dateSortValue(a.date)||String(b.updatedAt).localeCompare(String(a.updatedAt)))}
