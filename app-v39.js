...
function megaStoneFor(name){const n=normalizePokemonName(name);return MEGA_STONES[n]||(MEGA_NAMES.has(n)?'メガストーン':'')}
  ...
  function renderFilters(){const defs=[['all','全て','turquoise'],['red','注目','navy'],['green','上位','turquoise'],['black','環境','navy'],['blue','試用','pink'],['purple','研究','yellow']],host=$('#filters'),specialActive=state.someoneTeamsOnly||state.recentTeamsOnly;host.innerHTML=`<button class="btn small scope-filter ${state.teamScope==='my'&&!specialActive?'active':''}" data-scope-filter="my">My Team</button><button class="btn small scope-filter ${state.teamScope==='world'&&!specialActive?'active':''}" data-scope-filter="world">World Team</button>`+defs.map(([k,l,p])=>`<button class="btn small filter filter-palette-${p} ${state.filter===k&&!specialActive?'active':''}" data-filter="${k}">${l}</button>`).join('')+`<button class="btn small special-filter filter-palette-pink ${state.someoneTeamsOnly&&!state.recentTeamsOnly?'active':''}" data-someone-tab>誰か</button><button class="btn small special-filter filter-palette-yellow ${state.recentTeamsOnly?'active':''}" data-recent-tab>最近</button>`;host.querySelectorAll('[data-scope-filter]').forEach(b=>b.onclick=()=>{state.teamScope=b.dataset.scopeFilter;state.someoneTeamsOnly=false;state.recentTeamsOnly=false;scheduleSave();renderTeamScopeTabs();renderList();renderFilters()});host.querySelectorAll('[data-filter]').forEach(b=>b.onclick=()=>{state.filter=b.dataset.filter;state.someoneTeamsOnly=false;state.recentTeamsOnly=false;renderList();renderFilters()});host.querySelector('[data-someone-tab]').onclick=()=>{state.someoneTeamsOnly=!state.someoneTeamsOnly;state.recentTeamsOnly=false;renderList();renderFilters()};host.querySelector('[data-recent-tab]').onclick=()=>{state.recentTeamsOnly=!state.recentTeamsOnly;if(state.recentTeamsOnly)state.someoneTeamsOnly=false;renderList();renderFilters()}}
  function filtered(){
