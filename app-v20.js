...
function megaStoneFor(name){const n=normalizePokemonName(name);return MEGA_STONES[n]||(MEGA_NAMES.has(n)?'メガストーン':'')}
  ...
  function cloudPairingGroups(rows,legacy){return relationRows(rows,legacy).map(names=>({names}))}
  function cloudPayload(){const data=payload();return {...data,pairings:state.pairings.map(p=>({...p,counterGroups:cloudPairingGroups(p.counterGroups,p.counterPokemon),complementGroups:cloudPairingGroups(p.complementGroups,p.complementPokemon),strongAgainstGroups:cloudPairingGroups(p.strongAgainstGroups,p.strongAgainstPokemon)}))}}
  ...
  function registeredPokemonNames(){
    ...
    normalizePairingGroups();state.pairings.forEach(p=>[p.pokemon,...p.counterGroups.flat(),...p.complementGroups.flat(),...p.strongAgainstGroups.flat()].flat().forEach(n=>{n=String(n||'').trim();if(n)names.push(n)}));
  ...
  function relationRows(rows,legacy){const src=Array.isArray(rows)&&rows.length?rows:[legacy||[]];return src.map(row=>{const values=Array.isArray(row)?row:(Array.isArray(row?.names)?row.names:(Array.isArray(row?.pokemon)?row.pokemon:[]));return Array.from({length:6},(_,i)=>values[i]||'')})}
  function normalizePairingGroups(){state.pairings.forEach(p=>{p.counterGroups=relationRows(p.counterGroups,p.counterPokemon);p.complementGroups=relationRows(p.complementGroups,p.complementPokemon);p.strongAgainstGroups=relationRows(p.strongAgainstGroups,p.strongAgainstPokemon);p.counterPokemon=p.counterGroups[0]||Array(6).fill('');p.complementPokemon=p.complementGroups[0]||Array(6).fill('');p.strongAgainstPokemon=p.strongAgainstGroups[0]||Array(6).fill('');if(!pairingStatuses.includes(p.metaStatus))p.metaStatus='－'})}
  function pairingRelationHTML(title,key,rows,placeholder){return `<div class="pairing-relation-editor"><div class="pairing-relation-title"><label>${title}</label><button type="button" class="btn small pairing-add-row" data-add-relation="${key}">＋ 行を追加</button></div><div class="pairing-relation-rows">${relationRows(rows).map((names,index)=>`<div class="pairing-relation-row"><div class="pairing-row-number">${index+1}</div><div class="pairing-row-main"><input data-relation-key="${key}" data-relation-index="${index}" value="${esc(names.filter(Boolean).join(' '))}" placeholder="${placeholder}"><div class="pairing-relation-preview">${names.filter(Boolean).map(n=>`<div class="pairing-mon"><img data-pokemon="${esc(n)}" src="${imgFor(n)}"><small>${esc(n)}</small></div>`).join('')||'<span class="sub">名前を入力すると画像が出ます</span>'}</div></div>${index?`<button type="button" class="pairing-remove-row" data-remove-relation="${key}" data-remove-index="${index}" aria-label="この行を削除">×</button>`:''}</div>`).join('')}</div></div>`}
  ...
  function filteredPairings(){const q=searchNorm(state.pairingQuery||''),order={'注目':0,'流行':1,'再評価':2,'－':3};return [...state.pairings].filter(p=>state.pairingStatusFilter==='all'||p.metaStatus===state.pairingStatusFilter).filter(p=>!q||searchNorm([p.name,p.date,p.strength,p.selection,p.counter,p.memo,p.metaStatus,p.season,...p.pokemon,...p.counterGroups.flat(),...p.complementGroups.flat(),...p.strongAgainstGroups.flat()].join(' ')).includes(q)).sort((a,b)=>(order[a.metaStatus]??9)-(order[b.metaStatus]??9)||dateSortValue(b.date)-dateSortValue(a.date)||String(b.updatedAt).localeCompare(String(a.updatedAt)))}
  ...
  function renderPairingEditorV2(){
    ...
    document.querySelectorAll('[data-relation-key]').forEach(el=>{el.onchange=()=>{const key=el.dataset.relationKey,index=+el.dataset.relationIndex;p[key][index]=Array.from({length:6},(_,i)=>pairingNamesInput(el.value)[i]||'');p.updatedAt=new Date().toISOString();scheduleSave();renderPairingEditorV2()}});
    document.querySelectorAll('[data-add-relation]').forEach(btn=>{btn.onclick=()=>{p[btn.dataset.addRelation].push(Array(6).fill(''));p.updatedAt=new Date().toISOString();scheduleSave();renderPairingEditorV2()}});
  ...
  function exportData(){const blob=new Blob([JSON.stringify({version:'18',exportedAt:new Date().toISOString(),teams:state.teams,pairings:state.pairings,seasons:state.seasons},null,2)],{type:'application/json'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`pokemon-team-manager-v18-${new Date().toISOString().slice(0,10)}.json`;a.click();URL.revokeObjectURL(a.href)}
