...
function megaStoneFor(name){const n=normalizePokemonName(name);return MEGA_STONES[n]||(MEGA_NAMES.has(n)?'メガストーン':'')}
  ...
  function renderFilters(){const defs=[['all','すべて'],['red','🔴 注目'],['blue','🔵 試用'],['green','🟢 上位'],['purple','🟣 要研究']],host=$('#filters');host.innerHTML=defs.map(([k,l])=>`<button class="btn small filter ${state.filter===k?'active':''}" data-filter="${k}">${l}</button>`).join('')+`<button class="btn small someone-teams-tab ${state.someoneTeamsOnly&&!state.recentTeamsOnly?'active':''}" data-someone-tab>誰かの構築</button><button class="btn small recent-teams-btn ${state.recentTeamsOnly?'active':''}" data-recent-tab>最近使った構築</button>`;document.querySelectorAll('[data-filter]').forEach(b=>b.onclick=()=>{state.filter=b.dataset.filter;renderList();renderFilters()});host.querySelector('[data-someone-tab]').onclick=()=>{state.someoneTeamsOnly=!state.someoneTeamsOnly;state.recentTeamsOnly=false;renderList();renderFilters()};host.querySelector('[data-recent-tab]').onclick=()=>{state.recentTeamsOnly=!state.recentTeamsOnly;if(state.recentTeamsOnly)state.someoneTeamsOnly=false;renderList();renderFilters()}}
  function filtered(){
  ...
  function damageModifierSummary(data,def){const tidy=v=>Number(v).toFixed(3).replace(/0+$/,'').replace(/\.$/,''),out=[];if(Number(data.attackMod||1)!==1)out.push(`攻撃補正×${tidy(data.attackMod)}`);if(Number(def.defenseMod||1)!==1)out.push(`防御補正×${tidy(def.defenseMod)}`);if(data.stab)out.push('タイプ一致×1.5');if(data.spread)out.push('範囲技×0.75');if(data.burn)out.push('やけど×0.5');if(Number(data.damageMod||1)!==1)out.push(`ダメージ補正1×${data.damageMod}`);if(Number(data.damageMod2||1)!==1)out.push(`ダメージ補正2×${data.damageMod2}`);if(data.parentalBond)out.push('おやこあい');if(data.unseenFistQuarter)out.push('ふかしのこぶし×0.25');if(Number(def.defDamageMod||1)!==1)out.push(`被ダメ補正×${def.defDamageMod}`);if(def.resistBerry)out.push('半減実×0.5');if(def.sitrus)out.push('オボンのみ');if(def.fixedDamage&&def.fixedDamage!=='0')out.push(`定数ダメージ:${def.fixedDamage}`);return out.join(' ／ ')}
