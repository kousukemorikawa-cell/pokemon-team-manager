...
function megaStoneFor(name){const n=normalizePokemonName(name);return MEGA_STONES[n]||(MEGA_NAMES.has(n)?'メガストーン':'')}
  ...
  function renderFilters(){const defs=[['all','すべて'],['red','🔴 注目'],['blue','🔵 試用'],['green','🟢 上位'],['purple','🟣 要研究']],host=$('#filters');host.innerHTML=defs.map(([k,l])=>`<button class="btn small filter ${state.filter===k?'active':''}" data-filter="${k}">${l}</button>`).join('')+`<button class="btn small someone-teams-tab ${state.someoneTeamsOnly&&!state.recentTeamsOnly?'active':''}" data-someone-tab>誰かの構築</button>${state.someoneTeamsOnly?`<button class="btn small recent-teams-btn ${state.recentTeamsOnly?'active':''}" data-recent-tab>最近使った構築</button>`:''}`;document.querySelectorAll('[data-filter]').forEach(b=>b.onclick=()=>{state.filter=b.dataset.filter;renderList();renderFilters()});host.querySelector('[data-someone-tab]').onclick=()=>{state.someoneTeamsOnly=!state.someoneTeamsOnly;state.recentTeamsOnly=false;renderList();renderFilters()};host.querySelector('[data-recent-tab]')?.addEventListener('click',()=>{state.recentTeamsOnly=!state.recentTeamsOnly;renderList();renderFilters()})}
  function filtered(){
  ...
  function damageHistoryCard(h){const sides=historySides(h),plain=String(h.result||'').split('\n').filter(x=>!x.includes('急所')).join('\n');return `<article class="history-result-card damage-history-card"><div class="history-card-actions"><button class="history-restore-check" data-restore-damage="${h.id}" title="${h.form?'この計算を復元':'残っている項目を復元'}">✓</button><button class="history-delete-btn" data-delete-damage="${h.id}" title="この履歴を削除">削除</button></div><div class="damage-history-sides"><div><b>攻撃側</b><div>${historyPokemonImages(sides.attackers)}</div></div><span class="damage-history-arrow">→</span><div><b>防御側</b><div>${historyPokemonImages(sides.defenders)}</div></div></div><small>${esc(h.date)} ／ ${esc((h.moves||[]).filter(Boolean).join('・')||'技名未登録')}</small>${historyEffortHTML(h)}<div class="saved-damage-result">${h.resultHtml||esc(plain)}</div></article>`}
