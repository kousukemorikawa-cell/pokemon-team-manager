...
function megaStoneFor(name){const n=normalizePokemonName(name);return MEGA_STONES[n]||(MEGA_NAMES.has(n)?'メガストーン':'')}
  ...
  function damageHistoryCard(h){const sides=historySides(h);return `<article class="history-result-card damage-history-card"><div class="history-card-actions"><button class="history-restore-check" data-restore-damage="${h.id}" title="${h.form?'この計算を復元':'残っている項目を復元'}">✓</button><button class="history-delete-btn" data-delete-damage="${h.id}" title="この履歴を削除">削除</button></div><div class="damage-history-sides"><div><b>攻撃側</b><div>${historyPokemonImages(sides.attackers)}</div></div><span class="damage-history-arrow">→</span><div><b>防御側</b><div>${historyPokemonImages(sides.defenders)}</div></div></div><small>${esc(h.date)} ／ ${esc((h.moves||[]).filter(Boolean).join('・')||'技名未登録')}</small><div class="saved-damage-result">${h.resultHtml||esc(h.result)}</div></article>`}
