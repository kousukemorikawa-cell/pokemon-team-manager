...
function megaStoneFor(name){const n=normalizePokemonName(name);return MEGA_STONES[n]||(MEGA_NAMES.has(n)?'メガストーン':'')}
  ...
  function renderFilters(){const defs=[['all','すべて'],['red','🔴 注目'],['blue','🔵 試用'],['green','🟢 上位'],['purple','🟣 要研究']];$('#filters').innerHTML=defs.map(([k,l])=>`<button class="btn small filter ${state.filter===k?'active':''}" data-filter="${k}">${l}</button>`).join('');const someone=$('#someoneTeamsBtn');if(someone)$('#filters').appendChild(someone);let recent=$('#recentTeamsBtn');if(!recent){recent=document.createElement('button');recent.id='recentTeamsBtn';recent.className='btn small recent-teams-btn';recent.textContent='最近使った構築';recent.onclick=()=>{state.recentTeamsOnly=!state.recentTeamsOnly;renderList();renderFilters()}}recent.hidden=!state.someoneTeamsOnly;if(state.someoneTeamsOnly)$('#filters').appendChild(recent);recent.classList.toggle('active',state.recentTeamsOnly);document.querySelectorAll('[data-filter]').forEach(b=>b.onclick=()=>{state.filter=b.dataset.filter;renderList();renderFilters()})}
  function filtered(){
  ...
  function historyEffortHTML(h){const panels=h.form?.panels||[],atk=panels.filter(p=>p.kind==='atk'&&p.values?.pokemon).map(p=>`${esc(p.values.pokemon)}：${p.values.category==='special'?'C':'A'}${esc(p.values.offenseEv||0)}`),def=panels.filter(p=>p.kind==='def'&&p.values?.pokemon).map(p=>`${esc(p.values.pokemon)}：H${esc(p.values.hpEv||0)} B${esc(p.values.bEv||0)} D${esc(p.values.dEv||0)}`);if(!atk.length&&!def.length)return '';return `<div class="history-effort-summary"><div><b>攻撃側努力値</b>${atk.join(' ／ ')||'－'}</div><div><b>防御側努力値</b>${def.join(' ／ ')||'－'}</div></div>`}
  ...
  function renderDamageHistory(q){const n=searchNorm(q),rows=state.damageHistory.filter(h=>!n||searchNorm([...(h.attackers||[]),...(h.defenders||[]),...(h.pokemon||[]),...(h.moves||[])].join(' ')).includes(n));$('#damageHistoryList').innerHTML=rows.length?rows.map(damageHistoryCard).join(''):'<div class="empty">計算履歴がありません。</div>';$('#damageHistoryList').querySelectorAll('.saved-damage-result .damage-result-block,.saved-damage-result .damage-result-line').forEach(el=>{if(el.textContent.includes('急所'))el.remove()});hydrateImages($('#damageHistoryList'))}
