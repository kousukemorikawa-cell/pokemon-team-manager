(() => {
  ...
  function natureOptions(current){const boosted=Object.keys(NATURES).filter(n=>NATURES[n].up).sort((a,b)=>['A','B','C','D','S'].indexOf(NATURES[a].up)-['A','B','C','D','S'].indexOf(NATURES[b].up));const neutral=Object.keys(NATURES).filter(n=>!NATURES[n].up);return `<option value="">性格を選択</option>`+boosted.map(n=>`<option style="color:${natureColor(n)};font-weight:700" value="${n}" ${n===current?'selected':''}>${natureLabel(n)}</option>`).join('')+`<option disabled>──────── 無補正 ────────</option>`+neutral.map(n=>`<option style="color:#64748b" value="${n}" ${n===current?'selected':''}>${natureLabel(n)}</option>`).join('')}
