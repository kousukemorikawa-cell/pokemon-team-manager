...
function megaStoneFor(name){const n=normalizePokemonName(name);return MEGA_STONES[n]||(MEGA_NAMES.has(n)?'メガストーン':'')}
  ...
  function openPokemonSearch(){
    const modal=$('#pokemonSearchModal'),input=$('#pokemonIndexSearch');state.pokemonBoxType='all';renderPokemonQuickList();modal.classList.add('show');input.value='';input.focus();renderPokemonIndexSuggestions('');
  }
