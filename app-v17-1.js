...
function megaStoneFor(name){const n=normalizePokemonName(name);return MEGA_STONES[n]||(MEGA_NAMES.has(n)?'メガストーン':'')}
  ...
  const blankPairing=()=>({id:uid(),date:todayText(),name:'',pokemon:Array(6).fill(''),strength:'',selection:'先発：\n後発：',counter:'',counterPokemon:Array(6).fill(''),complementPokemon:Array(6).fill(''),strongAgainstPokemon:Array(6).fill(''),metaStatus:'注目',season:state.currentSeason==='all'?'未分類':state.currentSeason,memo:'',updatedAt:new Date().toISOString()});
  const pairingStatuses=['注目','流行','減少','消滅','再評価'];
  function pairingNamesInput(value){return splitInput(String(value||'')).slice(0,6)}
  function pairingRelationHTML(title,key,names,placeholder){return `<div class="pairing-relation-editor"><label>${title}</label><input data-pairing-quick="${key}" value="${esc((names||[]).filter(Boolean).join(' '))}" placeholder="${placeholder}"><div class="pairing-relation-preview">${(names||[]).filter(Boolean).map(n=>`<div class="pairing-mon"><img data-pokemon="${esc(n)}" src="${imgFor(n)}"><small>${esc(n)}</small></div>`).join('')||'<span class="sub">名前を入力すると画像が出ます</span>'}</div></div>`}
