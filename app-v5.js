(() => {
  ...
  const state={teams:[], imageMap:{}, seasons:['未分類'], currentSeason:'all', filter:'all', query:'', currentId:null};
  ...
  const TAGS={
    ...
    blue:{label:'試したい構築',emoji:'🔵',color:'#1e88e5'},
  ...
  const ITEM_SLUGS={
    'こだわりハチマキ':'choice-band','こだわりメガネ':'choice-specs','こだわりスカーフ':'choice-scarf','きあいのタスキ':'focus-sash','いのちのたま':'life-orb','たべのこし':'leftovers','とつげきチョッキ':'assault-vest','ゴツゴツメット':'rocky-helmet','ブーストエナジー':'booster-energy','オボンのみ':'sitrus-berry','ラムのみ':'lum-berry','カゴのみ':'chesto-berry','フィラのみ':'figy-berry','ウイのみ':'wiki-berry','マゴのみ':'mago-berry','バンジのみ':'aguav-berry','イアのみ':'iapapa-berry','じゃくてんほけん':'weakness-policy','からぶりほけん':'blunder-policy','ルームサービス':'room-service','ものまねハーブ':'mirror-herb','おんみつマント':'covert-cloak','クリアチャーム':'clear-amulet','パンチグローブ':'punching-glove','いかさまダイス':'loaded-dice','あつぞこブーツ':'heavy-duty-boots','ぼうじんゴーグル':'safety-goggles','しんかのきせき':'eviolite','くろいヘドロ':'black-sludge','ひかりのねんど':'light-clay','メンタルハーブ':'mental-herb','パワフルハーブ':'power-herb','しろいハーブ':'white-herb','レッドカード':'red-card','だっしゅつボタン':'eject-button','だっしゅつパック':'eject-pack','ふうせん':'air-balloon','こうかくレンズ':'wide-lens','フォーカスレンズ':'zoom-lens','ピントレンズ':'scope-lens','するどいツメ':'razor-claw','たつじんのおび':'expert-belt','ちからのハチマキ':'muscle-band','ものしりメガネ':'wise-glasses','かえんだま':'flame-orb','どくどくだま':'toxic-orb','でんきだま':'light-ball','しんぴのしずく':'mystic-water','もくたん':'charcoal','きせきのタネ':'miracle-seed','じしゃく':'magnet','とけないこおり':'never-melt-ice','くろいメガネ':'black-glasses','メタルコート':'metal-coat','シルクのスカーフ':'silk-scarf','ノーマルジュエル':'normal-gem','きれいなぬけがら':'shed-shell','あかいいと':'destiny-knot','ようせいのハネ':'fairy-feather','メガストーン':'key-stone'
  };
  const ITEM_ALIASES={
  ...
  function itemCandidates(query){const q=searchNorm(query);if(!q)return [];return Object.keys(ITEM_SLUGS).filter(n=>itemSearchText(n).includes(q)).sort((a,b)=>Number(searchNorm(b).startsWith(q))-Number(searchNorm(a).startsWith(q))||a.length-b.length||a.localeCompare(b,'ja')).slice(0,24)}
  ...
  const apiCache=new Map(), itemCache=new Map();
  ...
  function payload(){return {teams:state.teams,seasons:state.seasons,currentSeason:state.currentSeason,updatedAt:new Date().toISOString()}}
  ...
  const blankTeam=()=>({id:uid(),user:'',rank:'',season:state.currentSeason==='all'?(state.seasons[0]||'未分類'):state.currentSeason,regulation:'',title:'',oneLine:'',tag:'none',updatedAt:new Date().toISOString(),pokemon:Array(6).fill(''),items:Array(6).fill(''),pokemonChanges:[],moveChanges:[],adjustments:[],favorableMatchups:[],unfavorableMatchups:[],overallMemo:{lead:'',weak:'',improve:'',battle:'',free:''}});
  ...
  async function itemImage(name){const slug=itemSlug(name);if(!slug)return null;if(itemCache.has(slug))return itemCache.get(slug);try{const r=await fetch(`https://pokeapi.co/api/v2/item/${slug}`);if(!r.ok)throw Error(r.status);const d=await r.json();const src=d?.sprites?.default||null;itemCache.set(slug,src);return src}catch(e){return null}}
  ...
  function renderFilters(){const defs=[['all','すべて'],['red','🔴 注目'],['blue','🔵 試したい'],['green','🟢 上位'],['purple','🟣 要研究']];$('#filters').innerHTML=defs.map(([k,l])=>`<button class="btn small filter ${state.filter===k?'active':''}" data-filter="${k}">${l}</button>`).join('');document.querySelectorAll('[data-filter]').forEach(b=>b.onclick=()=>{state.filter=b.dataset.filter;renderList();renderFilters()})}
  function filtered(){const q=state.query.trim().toLowerCase();return state.teams.filter(t=>(state.currentSeason==='all'||(t.season||'未分類')===state.currentSeason)&&(state.filter==='all'||t.tag===state.filter)&&(!q||[t.user,t.rank,t.title,t.season,t.regulation,...t.pokemon].join(' ').toLowerCase().includes(q)))}
  function renderList(){const list=filtered();$('#counts').textContent=`${state.currentSeason==='all'?'全シーズン':state.currentSeason}：全${state.teams.filter(t=>state.currentSeason==='all'||(t.season||'未分類')===state.currentSeason).length}件 ／ 表示${list.length}件　${Object.entries(TAGS).slice(1).map(([k,v])=>`${v.emoji}${state.teams.filter(t=>t.tag===k).length}`).join(' ')}`;$('#teamList').innerHTML=list.length?list.map(teamRowHTML).join(''):'<div class="empty">構築がありません。「＋新規」から追加してください。</div>';hydrateImages($('#teamList'));bindRows()}
  function teamRowHTML(t){return `<article class="team-row" data-id="${t.id}" data-tag="${t.tag}">
    <div class="team-meta">
      ...
      <div class="field wide"><label>構築名</label><input data-k="title" value="${esc(t.title)}" placeholder="構築名・軸"></div>
      <button class="btn small detail-btn wide">詳細を見る</button>
    </div>
    ...
    <div class="row-actions"><button class="tag-btn" title="タグ">${TAGS[t.tag||'none'].emoji}</button><button class="btn drag-handle" title="長押しして並べ替え">≡</button></div>
  </article>`}
  ...
  function openDetail(id){state.currentId=id;$('#listView').classList.remove('active');$('#detailView').classList.add('active');renderDetail();scrollTo(0,0)}
  function current(){return state.teams.find(t=>t.id===state.currentId)}
  function renderDetail(){const t=current();if(!t)return;$('#detailTitle').textContent=t.title||'構築詳細';$('#detailContent').innerHTML=`
    <div class="section"><h3>基本情報</h3><div class="detail-grid">
      <div class="quick-input"><div class="field"><label>基本情報まとめ入力</label><input id="basicQuickInput" placeholder="使用者 / 順位 / 構築名　または　使用者、順位、構築名"></div><small>「/」または「、」で区切って入力すると、使用者・順位・構築名へ反映します。</small></div>
      ${field('使用者','user',t.user)}${field('順位','rank',t.rank)}${seasonField(t.season)}${field('レギュレーション','regulation',t.regulation)}
      ${field('構築名','title',t.title)}${field('一言メモ','oneLine',t.oneLine)}
    </div></div>
    <div class="section"><h3>パーティ</h3><div class="quick-input" style="margin-bottom:10px"><div class="field"><label>6体まとめて入力</label><input id="partyQuickInput" value="${esc(t.pokemon.filter(Boolean).join(' / '))}" placeholder="カイリュー / サーフゴー / …　または　カイリュー、サーフゴー、…"></div><small>「/」または「、」で区切ると6枠へ自動反映します。</small></div><div class="editor-party">${t.pokemon.map((n,i)=>editorMon(n,(t.items||[])[i]||'',i)).join('')}</div><div class="image-guide" style="margin-top:10px">画像はポケモン名から自動取得します。日本語の正式名称で入力してください（例：カイリュー）。画像フォルダや画像登録は不要です。</div></div>
    ${cardsSection('ポケモン候補','pokemonChanges',t.pokemonChanges,pokemonChangeCard,'＋候補を追加')}
    ${cardsSection('技候補','moveChanges',t.moveChanges,moveChangeCard,'＋技候補を追加')}
    ${cardsSection('調整','adjustments',t.adjustments,adjustmentCard,'＋調整を追加')}
    ...
    <div class="section"><h3>構築全体メモ</h3><div class="detail-grid">
  ...
  function editorMon(name,item,i){return `<div class="editor-mon"><div class="editor-image-wrap"><img data-pokemon="${esc(name)}" src="${imgFor(name)}"><img class="held-item-icon large" data-item="${esc(item)}" alt=""></div><input data-poke-index="${i}" value="${esc(name)}" placeholder="ポケモン名"><input data-item-index="${i}" value="${esc(item)}" placeholder="持ち物名"></div>`}
  ...
  function natureOptions(current){return `<option value="">性格を選択</option>`+Object.keys(NATURES).map(n=>`<option value="${n}" ${n===current?'selected':''}>${natureLabel(n)}</option>`).join('')}
  function adjustmentCard(x,i){const ev=x.ev||{},iv=x.iv||{};return `<div class="memo-card adjustment-card" data-card="adjustments" data-index="${i}"><div class="adjust-grid">
    <div class="field"><label>ポケモン</label><input data-c="pokemon" value="${esc(x.pokemon||'')}"><div class="preview compact"><img data-preview="pokemon" data-pokemon="${esc(x.pokemon||'')}" src="${imgFor(x.pokemon)}"></div></div><div class="field"><label>性格</label><select data-c="nature">${natureOptions(x.nature||'')}</select></div><div class="field"><label>特性</label><input data-c="ability" value="${esc(x.ability||'')}"></div><div class="field"><label>持ち物</label><input data-c="item" value="${esc(x.item||'')}"><div class="item-preview"><img data-item="${esc(x.item||'')}" alt=""></div></div><div class="field"><label>テラスタイプ</label><input data-c="tera" value="${esc(x.tera||'')}"></div><div class="field"><label>技構成</label><input data-c="moves" value="${esc(x.moves||'')}"></div></div>
    <div class="stat-entry"><strong>Lv50実数値計算</strong><small>個体値は0〜31（32段階）、努力値は0〜252で入力します。</small>
      ...
      <div class="stat-table"><span>努力値</span>${['H','A','B','C','D','S'].map(k=>`<input type="number" min="0" max="252" step="4" data-ev="${k}" value="${esc(ev[k]??0)}">`).join('')}</div>
      <div class="stat-table calculated"><span>実数値</span>${['H','A','B','C','D','S'].map(k=>`<output data-stat-output="${k}">－</output>`).join('')}</div>
      <div class="stat-note" data-stat-note>ポケモン名を入力すると種族値を取得して計算します。</div>
    </div>
    <div class="field"><label>調整意図・使用感</label><textarea data-c="memo">${esc(x.memo||'')}</textarea></div><div class="status-row"><button class="btn danger delete-card">削除</button></div></div>`}
  ...
  function bindDetail(){const t=current();hydrateImages($('#detailContent'));const bq=$('#basicQuickInput');if(bq)bq.onchange=()=>{const a=splitInput(bq.value);if(a[0]!=null)t.user=a[0];if(a[1]!=null)t.rank=a[1];if(a[2]!=null)t.title=a.slice(2).join(' / ');t.updatedAt=new Date().toISOString();scheduleSave();renderDetail()};const pq=$('#partyQuickInput');if(pq)pq.onchange=()=>{const a=splitInput(pq.value).slice(0,6);t.pokemon=Array.from({length:6},(_,i)=>a[i]||'');t.updatedAt=new Date().toISOString();scheduleSave();renderDetail()};document.querySelectorAll('[data-basic]').forEach(el=>el.oninput=()=>{setTeam(t.id,{[el.dataset.basic]:el.value});if(el.dataset.basic==='title')$('#detailTitle').textContent=el.value||'構築詳細'});document.querySelectorAll('[data-overall]').forEach(el=>el.oninput=()=>{t.overallMemo[el.dataset.overall]=el.value;t.updatedAt=new Date().toISOString();scheduleSave()});document.querySelectorAll('[data-poke-index]').forEach(el=>{el.onchange=()=>{const i=+el.dataset.pokeIndex;t.pokemon[i]=el.value.trim();t.items=t.items||Array(6).fill('');if(isMegaPokemon(t.pokemon[i]))t.items[i]='メガストーン';t.updatedAt=new Date().toISOString();scheduleSave();renderDetail()};attachAutocomplete(el,'pokemon')});document.querySelectorAll('[data-item-index]').forEach(el=>{const i=+el.dataset.itemIndex;if(isMegaPokemon(t.pokemon[i])){el.value='メガストーン';el.disabled=true;el.title='メガシンカのため持ち物はメガストーン固定です'}el.onchange=()=>{t.items=t.items||Array(6).fill('');t.items[i]=isMegaPokemon(t.pokemon[i])?'メガストーン':el.value.trim();t.updatedAt=new Date().toISOString();scheduleSave();renderDetail()};attachAutocomplete(el,'item')});
    document.querySelectorAll('[data-add-card]').forEach(b=>b.onclick=()=>{const k=b.dataset.addCard;if(k==='pokemonChanges')t[k].push({from:'',to:'',reason:'',status:'未検証'});if(k==='moveChanges')t[k].push({pokemon:'',fromMove:'',toMove:'',reason:'',status:'未検証'});if(k==='adjustments')t[k].push({pokemon:'',nature:'',ability:'',item:'',tera:'',moves:'',iv:{H:31,A:31,B:31,C:31,D:31,S:31},ev:{H:0,A:0,B:0,C:0,D:0,S:0},memo:''});if(k==='favorableMatchups'||k==='unfavorableMatchups')t[k].push({_key:k,linkedTeamId:'',title:'',source:'',pokemon:Array(6).fill(''),reason:'',plan:''});scheduleSave();renderDetail()});
    document.querySelectorAll('.memo-card').forEach(card=>{const key=card.dataset.card,idx=+card.dataset.index,item=t[key][idx];card.querySelectorAll('[data-c]').forEach(el=>{if(el.tagName==='SELECT')el.value=item[el.dataset.c]||'未検証';el.oninput=()=>{item[el.dataset.c]=el.value;t.updatedAt=new Date().toISOString();scheduleSave();const p=card.querySelector(`[data-preview="${el.dataset.c}"]`);if(p){p.dataset.pokemon=el.value;p.src=imgFor(el.value);hydrateImages(card)}if(key==='adjustments'){if(el.dataset.c==='item'){const it=card.querySelector('img[data-item]');if(it){it.dataset.item=el.value;hydrateImages(card)}}updateStatCard(card,item)}}});card.querySelectorAll('[data-match-poke]').forEach(el=>el.onchange=()=>{item.pokemon=item.pokemon||Array(6).fill('');item.pokemon[+el.dataset.matchPoke]=el.value.trim();item.linkedTeamId='';t.updatedAt=new Date().toISOString();scheduleSave();renderDetail()});card.querySelectorAll('[data-ev]').forEach(el=>el.oninput=()=>{item.ev=item.ev||{};item.ev[el.dataset.ev]=clampNum(el.value,0,252);scheduleSave();updateStatCard(card,item)});card.querySelectorAll('[data-iv]').forEach(el=>el.oninput=()=>{item.iv=item.iv||{};item.iv[el.dataset.iv]=clampNum(el.value,0,31);scheduleSave();updateStatCard(card,item)});if(key==='adjustments')updateStatCard(card,item);
      ...
      card.querySelectorAll('[data-c="pokemon"]').forEach(el=>attachAutocomplete(el,'pokemon',()=>{if(key==='adjustments'&&isMegaPokemon(el.value)){item.item='メガストーン';scheduleSave();renderDetail()}}));if(key==='pokemonChanges'){card.querySelectorAll('[data-c="from"],[data-c="to"]').forEach(el=>attachAutocomplete(el,'pokemon'))}if(key==='adjustments'){const itemInput=card.querySelector('[data-c="item"]');if(itemInput){if(isMegaPokemon(item.pokemon)){item.item='メガストーン';itemInput.value='メガストーン';itemInput.disabled=true;itemInput.title='メガシンカのため持ち物はメガストーン固定です'}attachAutocomplete(itemInput,'item')}}card.querySelectorAll('[data-match-poke]').forEach(el=>attachAutocomplete(el,'pokemon'));
      card.querySelector('.delete-card').onclick=()=>{if(confirm('この項目を削除しますか？')){t[key].splice(idx,1);scheduleSave();renderDetail()}}})}
  ...
  function calculateStats(base,item){const keys=['H','A','B','C','D','S'],out={};for(const k of keys){const b=Number(base[k]||0),iv=clampNum(item.iv?.[k]??31,0,31),ev=clampNum(item.ev?.[k]??0,0,252);if(k==='H')out[k]=Math.floor(((2*b+iv+Math.floor(ev/4))*50)/100)+60;else{const raw=Math.floor(((2*b+iv+Math.floor(ev/4))*50)/100)+5;out[k]=Math.floor(raw*natureMultiplier(item.nature,k))}}return out}
  async function updateStatCard(card,item){const note=card.querySelector('[data-stat-note]');if(!card.isConnected)return;if(note)note.textContent='種族値を取得中…';const d=await pokemonData(item.pokemon);if(!card.isConnected)return;if(!d){card.querySelectorAll('[data-stat-output]').forEach(o=>o.textContent='－');if(note)note.textContent='ポケモン名を確認してください。メガ・リージョンフォームにも対応しています。';return}const map={hp:'H',attack:'A',defense:'B','special-attack':'C','special-defense':'D',speed:'S'},base={};d.stats.forEach(x=>base[map[x.stat.name]]=x.base_stat);const vals=calculateStats(base,item);card.querySelectorAll('[data-stat-output]').forEach(o=>o.textContent=vals[o.dataset.statOutput]??'－');if(note)note.textContent=`種族値：H${base.H} A${base.A} B${base.B} C${base.C} D${base.D} S${base.S} ／ Lv50`;}
  ...
  function backToList(){$('#detailView').classList.remove('active');$('#listView').classList.add('active');state.currentId=null;renderList();renderFilters();scrollTo(0,0)}
  function exportData(){const blob=new Blob([JSON.stringify({version:4,exportedAt:new Date().toISOString(),teams:state.teams,seasons:state.seasons},null,2)],{type:'application/json'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`pokemon-teams-${new Date().toISOString().slice(0,10)}.json`;a.click();URL.revokeObjectURL(a.href)}
  ...
  async function loadCloudData(){
    try{const {doc,getDoc}=cloud.api;const snap=await getDoc(doc(cloud.db,'users',cloud.user.uid,'apps','pokemon-team-manager'));
      if(snap.exists()){
        ...
        if(useCloud){state.teams=d.teams||[];state.seasons=d.seasons||['未分類'];state.currentSeason=d.currentSeason||'all';await dbSet('data',payload());normalizeState();renderSeasons();renderFilters();renderList();if(state.currentId)renderDetail();toast('クラウドデータを読み込みました')}
  ...
  function normalizeState(){state.seasons=Array.isArray(state.seasons)&&state.seasons.length?state.seasons:['未分類'];state.teams=Array.isArray(state.teams)?state.teams:[];state.imageMap=state.imageMap||{};state.teams.forEach(t=>{t.season=t.season||'未分類';if(!state.seasons.includes(t.season))state.seasons.push(t.season);t.pokemon=Array.from({length:6},(_,i)=>(t.pokemon||[])[i]||'');t.items=Array.from({length:6},(_,i)=>(t.items||[])[i]||'');t.pokemonChanges=t.pokemonChanges||[];t.moveChanges=t.moveChanges||[];t.adjustments=(t.adjustments||[]).map(a=>({...a,iv:{H:31,A:31,B:31,C:31,D:31,S:31,...(a.iv||{})},ev:{H:0,A:0,B:0,C:0,D:0,S:0,...(a.ev||{})}}));t.favorableMatchups=(t.favorableMatchups||[]).map(x=>({...x,_key:'favorableMatchups',linkedTeamId:x.linkedTeamId||'',pokemon:Array.isArray(x.pokemon)?Array.from({length:6},(_,i)=>x.pokemon[i]||''):Array(6).fill('')}));t.unfavorableMatchups=(t.unfavorableMatchups||[]).map(x=>({...x,_key:'unfavorableMatchups',linkedTeamId:x.linkedTeamId||'',pokemon:Array.isArray(x.pokemon)?Array.from({length:6},(_,i)=>x.pokemon[i]||''):Array(6).fill('')}));t.overallMemo=t.overallMemo||{lead:'',weak:'',improve:'',battle:'',free:''}})}
  async function init(){await openDB();const saved=await dbGet('data');if(saved){state.teams=saved.teams||[];state.imageMap=saved.imageMap||{};state.seasons=saved.seasons||['未分類'];state.currentSeason=saved.currentSeason||'all'}normalizeState();renderSeasons();renderFilters();renderList();$('#searchInput').oninput=e=>{state.query=e.target.value;renderList()};$('#newTeamBtn').onclick=$('#floatingAdd').onclick=addTeam;$('#addSeasonBtn').onclick=addSeason;$('#renameSeasonBtn').onclick=renameSeason;$('#deleteSeasonBtn').onclick=deleteSeason;$('#backBtn').onclick=backToList;$('#duplicateBtn').onclick=duplicateCurrent;$('#deleteTeamBtn').onclick=deleteCurrent;$('#exportBtn').onclick=exportData;$('#importBtn').onclick=()=>$('#importFile').click();$('#importFile').onchange=e=>importData(e.target.files[0]);$('#closeTagModal').onclick=()=>$('#tagModal').classList.remove('show');$('#tagModal').onclick=e=>{if(e.target===$('#tagModal'))$('#tagModal').classList.remove('show')};
