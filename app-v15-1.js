...
,
 ...
 ,
  ...
  const blankTeam=()=>({id:uid(),user:'',owner:state.teamScope==='my'?'self':'other',rank:'',season:state.currentSeason==='all'?(state.seasons[0]||'未分類'):state.currentSeason,regulation:'',title:'',date:todayText(),tag:'none',updatedAt:new Date().toISOString(),pokemon:Array(6).fill(''),items:Array(6).fill(''),abilities:Array(6).fill(''),natures:Array(6).fill(''),pokemonChanges:[],moveChanges:[],adjustments:Array.from({length:6},blankAdjustment),favorableMatchups:[],unfavorableMatchups:[],overallMemo:{lead:'',weak:'',improve:'',battle:'',free:''}});
  ...
  function importData(file){if(!file)return;const r=new FileReader();r.onload=()=>{try{const d=JSON.parse(r.result);if(!Array.isArray(d.teams))throw Error();if(!confirm('現在のデータを読み込みデータで置き換えますか？'))return;state.teams=d.teams;state.imageMap=d.imageMap||state.imageMap||{};state.seasons=Array.isArray(d.seasons)&&d.seasons.length?d.seasons:['未分類'];state.currentSeason='all';state.sortOrder=d.sortOrder||'manual';state.teams.forEach(t=>{t.favorableMatchups=(t.favorableMatchups||[]).map(x=>({...x,_key:'favorableMatchups',linkedTeamId:x.linkedTeamId||'',pokemon:Array.isArray(x.pokemon)?Array.from({length:6},(_,i)=>x.pokemon[i]||''):Array(6).fill('')}));t.unfavorableMatchups=(t.unfavorableMatchups||[]).map(x=>({...x,_key:'unfavorableMatchups',linkedTeamId:x.linkedTeamId||'',pokemon:Array.isArray(x.pokemon)?Array.from({length:6},(_,i)=>x.pokemon[i]||''):Array(6).fill('')}));t.overallMemo=t.overallMemo||{lead:'',weak:'',improve:'',battle:'',free:''}});normalizeState();scheduleSave();renderSeasons();renderFilters();renderTeamScopeTabs();renderSortControl();renderList();toast('読み込みました')}catch{alert('正しいバックアップファイルではありません。')}};r.readAsText(file)}
