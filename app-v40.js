...
function megaStoneFor(name){const n=normalizePokemonName(name);return MEGA_STONES[n]||(MEGA_NAMES.has(n)?'メガストーン':'')}
  ...
  function renderDamageLines(lines){return lines.map(line=>{const m=line.match(/（([\d.]+)〜([\d.]+)%）/);if(!m)return `<div class="damage-result-line">${esc(line)}</div>`;return `<div class="damage-result-block"><div>${esc(line)}</div>${damageBarHTML(Number(m[1]),Number(m[2]))}</div>`}).join('')}
  ...
  async function calculateAndStoreDamage(){await calculateBattleDamage();const resultNode=$('#damageResult'),historyNode=resultNode?.cloneNode(true);historyNode?.querySelectorAll('.damage-result-block,.damage-result-line').forEach(el=>{if(el.textContent.includes('急所'))el.remove()});const result=historyNode?.innerText.trim()||'',resultHtml=historyNode?.innerHTML||'';if(!result||result.includes('計算できる'))return;const form=captureBattleForm(),attackers=form.panels.filter(p=>p.kind==='atk').map(p=>p.values.pokemon).filter(Boolean),defenders=form.panels.filter(p=>p.kind==='def').map(p=>p.values.pokemon).filter(Boolean),moves=form.panels.filter(p=>p.kind==='atk').map(p=>p.values.moveName).filter(Boolean);state.damageHistory.unshift({id:uid(),date:new Date().toLocaleString('ja-JP'),calcName:form.calcName||'',attackers,defenders,pokemon:[...attackers,...defenders],moves,result,resultHtml,form});scheduleSave();toast('計算履歴に保存しました')}
