(() => {
  ...
  const state={teams:[], imageMap:{}, seasons:['未分類'], currentSeason:'all', teamScope:'world', filter:'all', query:'', sortOrder:'manual', currentId:null, currentPokemon:'', detailReturn:null, historyPick:null};
...
function megaStoneFor(name){const n=normalizePokemonName(name);return MEGA_STONES[n]||(MEGA_NAMES.has(n)?'メガストーン':'')}
  ...
  function payload(){return {teams:state.teams,seasons:state.seasons,currentSeason:state.currentSeason,teamScope:state.teamScope,sortOrder:state.sortOrder,updatedAt:new Date().toISOString()}}
  ...
  const blankTeam=()=>({id:uid(),user:'',owner:state.teamScope==='my'?'self':'other',rank:'',season:state.currentSeason==='all'?(state.seasons[0]||'未分類'):state.currentSeason,regulation:'',title:'',date:todayText(),tag:'none',updatedAt:new Date().toISOString(),pokemon:Array(6).fill(''),items:Array(6).fill(''),abilities:Array(6).fill(''),natures:Array(6).fill(''),pokemonChanges:[],moveChanges:[],adjustments:Array.from({length:6},blankAdjustment),favorableMatchups:[],unfavorableMatchups:[],overallMemo:{lead:'先：\n後：',weak:'',improve:'',battle:'',free:''},partyView:'overview'});
  ...
  function partyOverviewMon(t,i){const name=t.pokemon[i]||'',item=t.items[i]||'',ability=t.abilities[i]||'',nature=t.natures[i]||'';return `<article class="party-overview-mon" data-party-card="${i}"><div class="overview-image-wrap"><img data-pokemon="${esc(name)}" src="${imgFor(name)}"><img class="held-item-icon large" data-item="${esc(item)}" alt=""></div><strong class="overview-name">${esc(name||'未登録')}</strong><div class="overview-line">${esc(item||'持ち物未登録')}｜${esc(ability||'特性未登録')}</div><div class="overview-nature ${natureClass(nature)}">${esc(natureLabel(nature)||'性格未登録')}</div><div class="party-stat-summary compact" data-party-stat="${i}">調整後：H－, A－, B－, C－, D－, S－</div><div class="party-effort-summary compact" data-party-effort="${i}">振り方：H0, A0, B0, C0, D0, S0</div></article>`}
  ...
  function importData(file){if(!file)return;const r=new FileReader();r.onload=()=>{try{const d=JSON.parse(r.result);if(!Array.isArray(d.teams))throw Error();if(!confirm('現在のデータを読み込みデータで置き換えますか？'))return;state.teams=d.teams;state.imageMap=d.imageMap||state.imageMap||{};state.seasons=Array.isArray(d.seasons)&&d.seasons.length?d.seasons:['未分類'];state.currentSeason='all';state.sortOrder=d.sortOrder||'manual';state.teams.forEach(t=>{t.favorableMatchups=(t.favorableMatchups||[]).map(x=>({...x,_key:'favorableMatchups',linkedTeamId:x.linkedTeamId||'',pokemon:Array.isArray(x.pokemon)?Array.from({length:6},(_,i)=>x.pokemon[i]||''):Array(6).fill('')}));t.unfavorableMatchups=(t.unfavorableMatchups||[]).map(x=>({...x,_key:'unfavorableMatchups',linkedTeamId:x.linkedTeamId||'',pokemon:Array.isArray(x.pokemon)?Array.from({length:6},(_,i)=>x.pokemon[i]||''):Array(6).fill('')}));t.partyView=t.partyView==='edit'?'edit':'overview';t.overallMemo=t.overallMemo||{lead:'先：\n後：',weak:'',improve:'',battle:'',free:''};if(!String(t.overallMemo.lead||'').trim())t.overallMemo.lead='先：\n後：'});normalizeState();scheduleSave();renderSeasons();renderFilters();renderTeamScopeTabs();renderSortControl();renderList();toast('読み込みました')}catch{alert('正しいバックアップファイルではありません。')}};r.readAsText(file)}
  ...
  function normalizeState(){state.seasons=Array.isArray(state.seasons)&&state.seasons.length?state.seasons:['未分類'];state.teams=Array.isArray(state.teams)?state.teams:[];state.imageMap=state.imageMap||{};state.teams.forEach(t=>{t.season=t.season||'未分類';t.owner=t.owner==='self'?'self':'other';t.date=normalizeDateText(t.date)||normalizeDateText(t.oneLine)||todayText();if(!state.seasons.includes(t.season))state.seasons.push(t.season);t.pokemon=Array.from({length:6},(_,i)=>(t.pokemon||[])[i]||'');t.items=Array.from({length:6},(_,i)=>(t.items||[])[i]||'');t.abilities=Array.from({length:6},(_,i)=>(t.abilities||[])[i]||((t.adjustments||[])[i]?.ability||''));t.natures=Array.from({length:6},(_,i)=>(t.natures||[])[i]||((t.adjustments||[])[i]?.nature||''));t.pokemonChanges=t.pokemonChanges||[];t.moveChanges=t.moveChanges||[];t.adjustments=Array.from({length:6},(_,i)=>{const a=(t.adjustments||[])[i]||blankAdjustment();return {...a,pokemon:t.pokemon[i],item:t.items[i],ability:t.abilities[i],nature:t.natures[i],iv:{H:31,A:31,B:31,C:31,D:31,S:31,...(a.iv||{})},ev:sanitizeEffortObject(Object.fromEntries(['H','A','B','C','D','S'].map(k=>[k,legacyEVToPoint(({H:0,A:0,B:0,C:0,D:0,S:0,...(a.ev||{})})[k])])) )}});t.favorableMatchups=(t.favorableMatchups||[]).map(x=>({...x,_key:'favorableMatchups',linkedTeamId:x.linkedTeamId||'',pokemon:Array.isArray(x.pokemon)?Array.from({length:6},(_,i)=>x.pokemon[i]||''):Array(6).fill('')}));t.unfavorableMatchups=(t.unfavorableMatchups||[]).map(x=>({...x,_key:'unfavorableMatchups',linkedTeamId:x.linkedTeamId||'',pokemon:Array.isArray(x.pokemon)?Array.from({length:6},(_,i)=>x.pokemon[i]||''):Array(6).fill('')}));t.partyView=t.partyView==='edit'?'edit':'overview';t.overallMemo=t.overallMemo||{lead:'先：\n後：',weak:'',improve:'',battle:'',free:''};if(!String(t.overallMemo.lead||'').trim())t.overallMemo.lead='先：\n後：'})}
