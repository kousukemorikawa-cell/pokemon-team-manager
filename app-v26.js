(() => {
  ...
  const state={teams:[], pairings:[], pokemonRelations:[], savedMoves:[], battleRecords:[], imageMap:{}, seasons:['未分類'], currentSeason:'all', teamScope:'world', filter:'all', query:'', sortOrder:'manual', currentId:null, currentPokemon:'', detailReturn:null, historyPick:null,currentPairingId:null,pairingMode:'browse',pairingReturnTeamId:null,pairingQuery:'',pairingStatusFilter:'all',pokemonReturnView:null,battleReturnTeamId:null};
...
function megaStoneFor(name){const n=normalizePokemonName(name);return MEGA_STONES[n]||(MEGA_NAMES.has(n)?'メガストーン':'')}
  ...
  function hideAllViews(){['listView','detailView','pokemonView','pairingView','pairingEditView','battleToolView'].forEach(id=>$('#'+id)?.classList.remove('active'))}
  ...
  function openBattleTool(selfTeamId=''){state.battleReturnTeamId=selfTeamId||state.battleReturnTeamId||'';hideAllViews();$('#battleToolView').classList.add('active');$('#battleToolContent').innerHTML=battleToolHTML();enhanceMultiplierSelectors();bindBattleTool();hydrateImages($('#battleToolContent'));scrollTo(0,0)}
  ...
  function battleDataSection(t){const rows=state.battleRecords.filter(r=>r.selfTeamId===t.id);return `<div class="section battle-data-section"><div class="section-title-row"><div><h3>対戦データ</h3><p class="sub">${rows.length}件保存されています。</p></div><button type="button" class="btn primary" data-open-battle-data>対戦データを見る</button></div><div class="battle-record-list">${rows.map(r=>{const o=state.teams.find(x=>x.id===r.opponentTeamId);return `<div class="battle-record"><b>${esc(r.date)} ／ ${esc(o?teamLabel(o):'削除済みの構築')}</b><p>${esc(r.memo)}</p></div>`}).join('')||'<div class="sub">まだ対戦データはありません。</div>'}</div></div>`}
  document.addEventListener('click',e=>{if(e.target?.closest?.('#battleToolBtn'))openBattleTool();if(e.target?.closest?.('[data-open-battle-data]'))openBattleTool(state.currentId)},true);
  function exportData(){const blob=new Blob([JSON.stringify({version:'26',exportedAt:new Date().toISOString(),teams:state.teams,pairings:state.pairings,pokemonRelations:state.pokemonRelations,savedMoves:state.savedMoves,battleRecords:state.battleRecords,seasons:state.seasons},null,2)],{type:'application/json'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`pokemon-team-manager-v26-${new Date().toISOString().slice(0,10)}.json`;a.click();URL.revokeObjectURL(a.href)}
  ...
  async function loadCloudData(){
    try{const {doc,getDoc}=cloud.api;const snap=await getDoc(doc(cloud.db,'users',cloud.user.uid,'apps','pokemon-team-manager'));
      if(snap.exists()){
        const d=snap.data();state.savedMoves=Array.isArray(d.savedMoves)?d.savedMoves:[];state.battleRecords=Array.isArray(d.battleRecords)?d.battleRecords:[];
