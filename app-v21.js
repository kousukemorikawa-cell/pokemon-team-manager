(() => {
  ...
  const state={teams:[], pairings:[], pokemonRelations:[], imageMap:{}, seasons:['未分類'], currentSeason:'all', teamScope:'world', filter:'all', query:'', sortOrder:'manual', currentId:null, currentPokemon:'', detailReturn:null, historyPick:null,currentPairingId:null,pairingMode:'browse',pairingReturnTeamId:null,pairingQuery:'',pairingStatusFilter:'all',pokemonReturnView:null};
...
function megaStoneFor(name){const n=normalizePokemonName(name);return MEGA_STONES[n]||(MEGA_NAMES.has(n)?'メガストーン':'')}
  ...
  function dbGet(key){return new Promise((resolve,reject)=>{const r=db.transaction(STORE).objectStore(STORE).get(key);r.onsuccess=()=>{if(key==='data')state.pokemonRelations=Array.isArray(r.result?.pokemonRelations)?r.result.pokemonRelations:[];resolve(r.result)};r.onerror=()=>reject(r.error)})}
  ...
  function payload(){return {teams:state.teams,pairings:state.pairings,pokemonRelations:state.pokemonRelations,seasons:state.seasons,currentSeason:state.currentSeason,teamScope:state.teamScope,sortOrder:state.sortOrder,updatedAt:new Date().toISOString()}}
  function cloudPairingGroups(rows,legacy){return relationEntries(rows,legacy).map(row=>({names:row.names,memo:row.memo||''}))}
  function cloudPayload(){const data=payload();return {...data,pairings:state.pairings.map(p=>({...p,counterGroups:cloudPairingGroups(p.counterGroups,p.counterPokemon),complementGroups:cloudPairingGroups(p.complementGroups,p.complementPokemon),strongAgainstGroups:cloudPairingGroups(p.strongAgainstGroups,p.strongAgainstPokemon)})),pokemonRelations:state.pokemonRelations.map(r=>({...r,counterGroups:cloudPairingGroups(r.counterGroups,[])}))}}
  ...
  function registeredPokemonNames(){
    ...
    normalizePokemonRelations();state.pokemonRelations.forEach(r=>{if(r.name)names.push(r.name);relationRows(r.counterGroups).flat().forEach(n=>{n=String(n||'').trim();if(n)names.push(n)})});
  ...
  function hideAllViews(){['listView','detailView','pokemonView','pairingView','pairingEditView'].forEach(id=>$('#'+id)?.classList.remove('active'))}
  ...
  async function openPokemonPage(name,returnView=null){
    state.currentPokemon=name;state.pokemonReturnView=returnView;hideAllViews();$('#pokemonView').classList.add('active');$('#pokemonPageTitle').textContent=state.historyPick?`${name}：過去の型を選択`:`${name}のまとめ`;await renderPokemonPage();scrollTo(0,0)
  }
  ...
  function pokemonPairingSectionsHTML(name){const r=pairingRelationsForPokemon(name),section=(title,rows,cls)=>`<div class="section pokemon-meta-section ${cls}"><h3>${title}</h3><div class="pokemon-pairing-list">${rows.length?rows.map(pokemonPairingCard).join(''):'<div class="empty compact">該当する並びはありません。</div>'}</div></div>`;return `<div class="pokemon-meta-summary"><h2>メタ研究とのつながり</h2>${section('このポケモンを含む並び',r.main,'main')}${section('このポケモンが対策になる並び',r.counter,'counter')}${section('このポケモンが補完になる並び',r.complement,'complement')}${section('このポケモンに対して有利な並び',r.against,'against')}</div>`}
  function normalizePokemonRelations(){state.pokemonRelations=Array.isArray(state.pokemonRelations)?state.pokemonRelations:[];state.pokemonRelations=state.pokemonRelations.map(r=>({name:String(r?.name||''),counterGroups:relationEntries(r?.counterGroups||[],[])}))}
  function pokemonRelationRecord(name){normalizePokemonRelations();let record=state.pokemonRelations.find(r=>normalizePokemonName(r.name)===normalizePokemonName(name));if(!record){record={name,counterGroups:[{names:Array(6).fill(''),memo:''}]};state.pokemonRelations.push(record)}return record}
  function pokemonDirectEditorHTML(name){const record=pokemonRelationRecord(name);return `<div class="section pokemon-direct-editor"><div class="pairing-relation-title"><div><h3>対策ポケモン</h3><p class="sub">${esc(name)}への対策を直接登録できます。</p></div><button type="button" class="btn small pairing-add-row" data-add-pokemon-counter>＋ 行を追加</button></div><div class="pokemon-direct-rows">${relationEntries(record.counterGroups).map((row,index)=>`<div class="pokemon-direct-row"><div class="pairing-row-number">${index+1}</div><div class="pokemon-direct-left"><input data-pokemon-counter-index="${index}" value="${esc(row.names.filter(Boolean).join('、'))}" placeholder="、で区切って打ってください"><div class="pairing-relation-preview">${row.names.filter(Boolean).map(n=>`<div class="pairing-mon"><img data-pokemon="${esc(n)}" src="${imgFor(n)}"><small>${esc(n)}</small></div>`).join('')||'<span class="sub">名前を入力すると画像が出ます</span>'}</div></div><textarea data-pokemon-counter-memo="${index}" placeholder="この対策についてのメモ">${esc(row.memo)}</textarea>${index?`<button type="button" class="pairing-remove-row" data-remove-pokemon-counter="${index}" aria-label="この行を削除">×</button>`:''}</div>`).join('')}</div></div>`}
  function bindPokemonDirectEditor(name){const record=pokemonRelationRecord(name);document.querySelectorAll('[data-pokemon-counter-index]').forEach(el=>{el.onchange=()=>{const i=+el.dataset.pokemonCounterIndex;record.counterGroups[i]={...(record.counterGroups[i]||{}),names:Array.from({length:6},(_,j)=>pairingNamesInput(el.value)[j]||''),memo:String(record.counterGroups[i]?.memo||'')};scheduleSave();renderPokemonPage()}});document.querySelectorAll('[data-pokemon-counter-memo]').forEach(el=>{el.oninput=()=>{const i=+el.dataset.pokemonCounterMemo;record.counterGroups[i]={...(record.counterGroups[i]||{}),names:relationEntries([record.counterGroups[i]])[0].names,memo:el.value};scheduleSave()};enableAutoGrow(el)});document.querySelectorAll('[data-add-pokemon-counter]').forEach(btn=>btn.onclick=()=>{record.counterGroups.push({names:Array(6).fill(''),memo:''});scheduleSave();renderPokemonPage()});document.querySelectorAll('[data-remove-pokemon-counter]').forEach(btn=>btn.onclick=()=>{record.counterGroups.splice(+btn.dataset.removePokemonCounter,1);scheduleSave();renderPokemonPage()})}
  ...
  function closePokemonPage(){if(state.historyPick){const pick=state.historyPick,ctx=pick.returnContext;state.historyPick=null;openDetail(pick.teamId,ctx);return}if(state.pokemonReturnView==='pairing'){state.pokemonReturnView=null;openPairingList('browse');return}state.pokemonReturnView=null;hideAllViews();$('#listView').classList.add('active');renderList();renderFilters();scrollTo(0,0)}

  function addTeam(){const t=blankTeam();state.teams.unshift(t);scheduleSave();openDetail(t.id)}

  const blankPairing=()=>({id:uid(),date:todayText(),name:'',pokemon:Array(6).fill(''),strength:'',selection:'先発：\n後発：',counter:'',counterPokemon:Array(6).fill(''),complementPokemon:Array(6).fill(''),strongAgainstPokemon:Array(6).fill(''),counterGroups:[Array(6).fill('')],complementGroups:[Array(6).fill('')],strongAgainstGroups:[Array(6).fill('')],metaStatus:'－',season:state.currentSeason==='all'?'未分類':state.currentSeason,memo:'',updatedAt:new Date().toISOString()});
  ...
  function renderPairingEditorV2(){
    ...
    document.querySelectorAll('[data-pairing-pokemon]').forEach(el=>{attachAutocomplete(el,'pokemon');el.onchange=()=>{p.pokemon[+el.dataset.pairingPokemon]=el.value.trim();if(!p.name)p.name=p.pokemon.filter(Boolean).join('＋');p.updatedAt=new Date().toISOString();scheduleSave();renderPairingEditorV2()}});
    document.querySelectorAll('[data-pairing-quick="pokemon"]').forEach(el=>{el.onchange=()=>{p.pokemon=Array.from({length:6},(_,i)=>pairingNamesInput(el.value)[i]||'');if(!p.name)p.name=p.pokemon.filter(Boolean).join('＋');p.updatedAt=new Date().toISOString();scheduleSave();renderPairingEditorV2()}});
    document.querySelectorAll('[data-relation-key]').forEach(el=>{el.onchange=()=>{const key=el.dataset.relationKey,index=+el.dataset.relationIndex;p[key][index]={...(p[key][index]||{}),names:Array.from({length:6},(_,i)=>pairingNamesInput(el.value)[i]||''),memo:String(p[key][index]?.memo||'')};p.updatedAt=new Date().toISOString();scheduleSave();renderPairingEditorV2()}});
    document.querySelectorAll('[data-relation-memo-key]').forEach(el=>{el.oninput=()=>{const key=el.dataset.relationMemoKey,index=+el.dataset.relationMemoIndex;p[key][index]={...(p[key][index]||{}),names:relationEntries([p[key][index]])[0].names,memo:el.value};p.updatedAt=new Date().toISOString();scheduleSave()};enableAutoGrow(el)});
  ...
  function exportData(){const blob=new Blob([JSON.stringify({version:'21',exportedAt:new Date().toISOString(),teams:state.teams,pairings:state.pairings,pokemonRelations:state.pokemonRelations,seasons:state.seasons},null,2)],{type:'application/json'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`pokemon-team-manager-v21-${new Date().toISOString().slice(0,10)}.json`;a.click();URL.revokeObjectURL(a.href)}
