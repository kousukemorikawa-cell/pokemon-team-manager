...
function megaStoneFor(name){const n=normalizePokemonName(name);return MEGA_STONES[n]||(MEGA_NAMES.has(n)?'メガストーン':'')}
  ...
  function prepareDamageCalculator(root){enhanceMultiplierSelectors();root.querySelectorAll('[data-damage-atk="1"],[data-damage-def="1"]').forEach(p=>p.classList.add('damage-panel-secondary','is-hidden'));root.querySelectorAll('[data-toggle-damage-panel]').forEach(btn=>btn.onclick=()=>{const panel=root.querySelector(`[data-damage-${btn.dataset.toggleDamagePanel}="1"]`),show=panel.classList.toggle('is-hidden');btn.textContent=show?`＋ ${btn.dataset.toggleDamagePanel==='atk'?'攻撃':'防御'}側2匹目を追加`:'− 2匹目を閉じる'});fillSavedMoveSearch();root.querySelectorAll('[data-dmg="moveName"]').forEach(input=>{input.setAttribute('list','savedMoveNames');input.placeholder='登録技を検索／技名を入力';input.addEventListener('change',()=>{const m=state.savedMoves.find(x=>searchNorm(x.name)===searchNorm(input.value));if(!m)return;const p=input.closest('[data-damage-atk]');p.querySelector('[data-dmg="power"]').value=m.power;p.querySelector('[data-dmg="category"]').value=m.category;p.querySelector('[data-dmg="moveType"]').value=m.type})});bindDamagePanels(root);enhanceDamagePreviews(root);hydrateImages(root)}
  ...
  function applyPartySetToDamage(btn){const side=btn.dataset.partyDamageSide,name=btn.dataset.partyDamagePokemon,team=state.teams.find(t=>t.id===btn.dataset.partyTeam),index=Number(btn.dataset.partyIndex),adjustment=team?.adjustments?.[index]||{};setDamagePokemon(side,name);const panel=[...document.querySelectorAll(`[data-damage-${side}]`)].find(p=>p.querySelector('[data-dmg="pokemon"]')?.value===name);if(!panel)return;if(side==='atk'){const category=panel.querySelector('[data-dmg="category"]')?.value||'physical',key=category==='special'?'C':'A';panel.querySelector('[data-dmg="offenseEv"]').value=adjustment.ev?.[key]||0}else{panel.querySelector('[data-dmg="hpEv"]').value=adjustment.ev?.H||0;panel.querySelector('[data-dmg="bEv"]').value=adjustment.ev?.B||0;panel.querySelector('[data-dmg="dEv"]').value=adjustment.ev?.D||0}updateDamagePanelPreview(panel);toast(`${name}の登録型を反映しました`)}
